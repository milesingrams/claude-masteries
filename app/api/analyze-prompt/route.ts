...
export async function POST(req: Request) {
  try {
    ...
    if (!partial_prompt || partial_prompt.trim().length < 10) {
...
2. Surface 0-3 relevant masteries (prefer high-impact, avoid overwhelming)
3. For active chips, check if the prompt now satisfies their satisfaction criteria
4. Be conservative - only surface chips that are clearly relevant
5. Prefer one high-relevance chip over multiple medium ones
